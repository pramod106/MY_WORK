    ...
    while(j<np.size(x)):
        ...
        while(state == prev_state):
            ...
            while(tu<=T_f):
                ...
                # omega_ref = kk*tempu5
                # print omega_ref
                controlled_omega = max( min(omega_ref + ( ((c2*tempu5*2.7)*(ye*math.cos(theta_e/2) - xe*math.sin(theta_e/2) ))/( math.sqrt( 1 + (pow(xe,2)) + (pow(ye,2)))) ) + c3*math.sin(theta_e/2) ,0.7157) , -0.7157) #2nd
                # controlled_omega = max(min(omega_ref + k_theta*theta_e + tempu5*2.7*k_y*(math.sin(theta_e) / theta_e),0.7157 ),-0.7157) #3rd
                 
                delta = max(min(math.atan((wheelBase*controlled_omega*1.33)/(control_velocity)),0.261799),-0.261799) # delta = steering angle
                # delta = abs(delta)
